...
//        }
    ...
    inner class CustomWebSocketListener : WebSocketListener() {
        ...
        override fun onMessage(webSocket: WebSocket, text: String) {
            ...
            val json = getJsonArrayFromString(text)
            json.forEach {
                val id = it["id"]
                val status = it["status"]
                val updated = it["updated"]
                val text = it["text"]
                val new = TodoEntity(id as Int, text as String, updated as Long, status as String)
                viewModel.add(new)
            }
//
...
//            }
        ...
        fun getJsonArrayFromString(text: String) : JsonArray<JsonObject> {
